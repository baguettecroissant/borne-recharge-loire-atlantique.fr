#!/usr/bin/env node
/**
 * Verify content uniqueness across all local pages in Loire-Atlantique (44).
...
for (const category of CATEGORIES) {
  ...
  for (const c of communes) {
    const combo = [
      ...
      c.distanceNantes <= 5 ? 'core' : c.distanceNantes <= 15 ? 'inner' : c.distanceNantes <= 35 ? 'mid' : 'outer',
...
for (const c of communes) {
  ...
  distanceSet.add(c.distanceNantes);
