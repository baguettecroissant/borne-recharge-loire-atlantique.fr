...
async function fetchCities() {
  const url = 'https://geo.api.gouv.fr/departements/44/communes?fields=nom,code,population,codesPostaux,centre';
  
  console.log('Fetching Loire-Atlantique (44) communes from Geo API...');
  try {
    ...
    // Process and filter communes in Loire-Atlantique (44) with population >= 2000
    const processed = data
      .map(c => {
        let lat = 47.218371; // Nantes lat
        let lng = -1.553621; // Nantes lng
